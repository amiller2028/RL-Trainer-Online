...
THREE.Car = function () {
  ...
  this.loadPartsTest = function (bodyMesh, wheelMesh){
    createBody(bodyMesh.geometry, bodyMesh.material);
    createWheels(wheelMesh.geometry, wheelMesh.material);
  };
	this.updateCarModel = function ( delta, controls ) {
		...
		if ( ! ( controls.moveForward || controls.moveBackward ) ) {
			...
			if ( this.speed > 0 ) {
				...
				var k = window.game.helpers.exponentialEaseOut( this.speed / this.MAX_SPEED );
				...
				var k = window.game.helpers.exponentialEaseOut( this.speed / this.MAX_REVERSE_SPEED );
